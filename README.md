...
I measured ~6.3 uA when the LIS2DW12 is awake in LPMode 2 at 25 Hz, ~4.8 uA when it is asleep in LPMode 2 at 12.5 Hz and the Ladybug uses ~2.2 uA in STOP mode. So the LIS2DW12 is using ~4.1 uA (@3V3) when awake in LPMode2 at 25 Hz compared to the 3 uA (@1V8) expected from Table 10 in the data sheet and ~2.6 uA (@3V3) when alseep in LPMode 2 at 12.5 Hz instead of the 1.6 uA (@1V8) from the data sheet. So there is a ~1 uA greater current usage than expected which could be due to 1) using 3V3 instead of 1V8, 2) using the FIFO, or 3) something else. It is possible to cut sleep current even more by moving to LPMode1, which is limited to 12-bit data instead of the 14-bit available in all of the other modes.
