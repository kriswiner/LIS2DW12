...
void LIS2DW12::init(uint8_t fs, uint8_t odr, uint8_t mode, uint8_t lpMode, uint8_t bw, bool lowNoise)
{
   ...
   // 6D orientation detect (bit 7), single tap (bit 6) and double tap (bit 3) recognition routed to interrupt 1
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_CTRL4_INT1_PAD_CTRL, 0x80 | 0x40 | 0x08); 
   // data ready (bit 0) routed to interrupt 2
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_CTRL5_INT2_PAD_CTRL, 0x01); 
   
   // set 6D orientation threshold (bits 5-6), set x-axis tap threshold (bits 0 - 4) n x FS/32
   //  60 degree 6D threshold, 9 x 2/32 = 0.56 g tap threshold   
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_TAP_THS_X, 0x40 | 0x09); 
   //  z-axis priority (bits 5 - 7), 9 x 2/32 = 0.56 g tap threshold
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_TAP_THS_Y, 0xC0 | 0x09);
   //  enable tap detect on all three axes (bits 5 - 7), 9 x 2/32 = 0.56 g tap threshold
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_TAP_THS_Z, 0xE0 | 0x09); 
   //  double tap latency time (bits 4 - 7) 0 LSB = 16/ODR, 1 LSB = 32/ODR so 0.08s minimum at 400 Hz
   //  set quiet time (bits 2-3), 0 LSB = 2/ODR, 1 LSB = 4/ODR so 0.005s minimum at 400 Hz
   //  set shock time (bits 0-1), 0 LSB = 4/ODR, 1 LSB = 8/ODR so 0.01s minimum at 400 Hz
   //  set quiet time to 3*4/ODR = 0.06s, set shock time to 3*8/ODR = 0.06s, latency to 7*32/ODR = 0.66 s 
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_INT_DUR, 0x70 | 0x0C | 0x03);  //latency | quiet | shock
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_WAKE_UP_THS, 0x80); // single and double tap enabled
...
/* end of self test*/
  ...
  uint8_t LIS2DW12::get6DSource( ) {
  uint8_t temp = _i2c_bus->readByte(LIS2DW12_ADDRESS, LIS2DW12_SIXD_SRC);    // Read 6D source register  
  return temp;
  }

  
  uint8_t LIS2DW12::getTapSource( ) {
  uint8_t temp = _i2c_bus->readByte(LIS2DW12_ADDRESS, LIS2DW12_TAP_SRC);    // Read tape source register  
  return temp;
  }


  uint8_t LIS2DW12::getAllIntSource( ) {
  uint8_t temp = _i2c_bus->readByte(LIS2DW12_ADDRESS, LIS2DW12_ALL_INT_SRC);    // Read all INT source register  
  return temp;
  }
