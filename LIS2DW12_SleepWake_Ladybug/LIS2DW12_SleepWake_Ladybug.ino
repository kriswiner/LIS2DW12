...
ODR      odr    = LIS2DW12_ODR_25Hz;       // choices are 12.5, 25, 50, 100, 200, 400, 800, 1600 Hz, 1.6 Hz in lpMode, and POWERDOWN
...
void loop()
{
  ...
  if(LIS2DW12_wake_flag)
  {
   ...
   if(FIFOstatus & 0x80) {                            // if the FIFO threshold is reached
      numFIFOSamples =  FIFOstatus & 0x3F;
   ...
   digitalWrite(myLed, HIGH);  // turn on red led when motion detected
   }
  ...
  if(LIS2DW12_sleep_flag)
  {
   ...
   digitalWrite(myLed, LOW);                   // turn off red led when no motion
